...
<?php
    ...
    if(empty($developerid)||empty($managerid)||empty($reason)||empty($loanamount)||empty($loanperiod)||empty($current_date))
    {
        ...
        echo "<script> location.href='Developer_LoanApplication.php'; </script>" ;// default page
    }
    else
    {
        ...
        {
            $sql="SELECT * FROM loan_table WHERE Developer_ID='$developerid' and Status='Accepted' and Active='yes'";
            $result = mysqli_query($con, $sql);

    ...
    else if(strlen($phone_number) == 11)
    {
        if($type_of_employee == "Developer" )
        {
            ...
            if($count_cnic == 1)
		    {
                ...
                if($count_manager == 0)
                {
                    $sql = "SELECT * FROM developer_info where Email='$email' and Developer_ID !='$ID'";
                    $check_query = mysqli_query($con, $sql);
                    ...
                    if($count_developer == 0)
                    {
                        ...
                        if($count_admin == 0)
                        {
                            $sql="UPDATE developer_info SET Phone_Number='$phone_number',Email='$email',Password='$password',Address='$address'

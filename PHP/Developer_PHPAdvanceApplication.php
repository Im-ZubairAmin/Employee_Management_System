...
<?php
    ...
    if(empty($developerid)||empty($managerid)||empty($reason)||empty($advanceamount)||empty($advanceamount)||empty($current_date))
    {
        ...
        echo "<script> location.href='Developer_AdvanceApplication.php'; </script>" ;// default page
    }
    else
    {
        $sql="SELECT * FROM developer_info WHERE Developer_ID='$developerid'";
        ...
        {
            $sql="SELECT * FROM advance_table WHERE Developer_ID='$developerid' and Status='Accepted' and Active='yes'";
            $result = mysqli_query($con, $sql);

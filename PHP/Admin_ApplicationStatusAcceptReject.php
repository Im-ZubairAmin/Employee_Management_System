...
<?php
    ...
    else
    {
        ...
        if($Type == 'Advance')
        {
            ...
            {
                echo "<script> location.href='Admin_AdvanceApplication.php'; </script>" ;// default page
            }   
            else
            {
                echo "<script> location.href='Admin_AdvanceApplication.php'; </script>" ;// default page
        ...
        if($Type == 'Loan')
        {
            ...
            if(mysqli_query($con, $sql))
            {
                echo "<script> location.href='Admin_LoanApplication.php'; </script>" ;// default page
            }   
            else
            {
                echo "<script> location.href='Admin_LoanApplication.php'; </script>" ;// default page
        ...
        if($Type == 'Leave')
        {
            ...
            if($run_query = mysqli_query($con, $sql))
            {
                ...
                $DevID = $row["Manager_ID"];
                $sql="SELECT * FROM manager_info WHERE Manager_ID='$DevID'";
                if($run_query = mysqli_query($con, $sql))
                {
                    ...
                    if($row1['Total_Leave_Days'] >= $row1['Leave_Days_Used'] + $row['NumberOfDays'])
                    {
                        ...
                        if(mysqli_query($con, $sql))
                        {
                            ...
                            $sql="UPDATE manager_info SET Leave_Days_Used='$NewLeaveDays' WHERE Manager_ID='$DevID'";
                            if(mysqli_query($con, $sql))
                            {
                                echo "<script> location.href='Admin_LeaveApplication.php'; </script>" ;// default page
                            }   
                            else
                            {
                                echo "<script> location.href='Admin_LeaveApplication.php'; </script>" ;// default page
